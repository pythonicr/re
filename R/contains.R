...
#'
#' `re_contains` checks whether a specified pattern (regular expression) is
#' found within each element of a character vector. If the provided pattern is
#' not already a compiled pattern object, it compiles it using `re_compile`.
#'
#' @param pattern A regular expression pattern or a compiled pattern object.
#' @param string A character vector where each element is a string to be checked
#' against the pattern.
...
#' each element contains a match for the pattern.
